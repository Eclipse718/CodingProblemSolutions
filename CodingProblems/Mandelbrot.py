...
def draw_mandelbrot_path(screen, c, iter):
    ...
    for i in range(iter):
        ...
        if 0 <= x < width and 0 <= y < height:
            ...
            prev_x, prev_y = x, y
            last_valid_x, last_valid_y = x, y  # Update the last valid coordinates
        else:
            # Draw the line to the last valid point and then break the loop
            if last_valid_x is not None and last_valid_y is not None:
                if prev_x is not None and prev_y is not None:
                    pygame.draw.line(screen, (255, 255, 255), (prev_x, prev_y), (last_valid_x, last_valid_y), 1)
            break
